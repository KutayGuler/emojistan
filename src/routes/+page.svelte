...
<main>
	<div class="absolute top-0 flex h-full w-full scale-105 flex-wrap">
		<!-- class:selected={$currentEmoji == emoji}
    on:click={() => pickEmoji(emoji)} -->
		{#each Object.keys(emojis) as category}
			{#each shuffleArray(emojis[category]) as [emoji, _]}
				<div class="menu-emoji text-2xl">
					{emoji}
				</div>
			{/each}
		{/each}
	</div>
	<div class="aside mt-8 flex flex-col gap-2">
		<h1>Emojistan</h1>
		<button class="btn-primary btn w-full">PLAY</button>
		<button class="btn-secondary btn w-full">TUTORIAL</button>
	</div>
</main>
