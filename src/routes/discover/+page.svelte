...
<div class="tabs tabs-boxed py-3">
	...
	<button
		...
		class="tab {filter === 'following' ? 'bg-accent text-accent-content' : ''}"
		>Following</button
	>
	<button
		on:click={() => (filter = 'search')}
		class="tab {filter === 'search' ? 'bg-accent text-accent-content' : ''}"
		>Search</button
	>
</div>
{#if filter == 'search'}
